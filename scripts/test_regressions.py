...
class RegressionTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root=None
        self.source=ROOT/'plugin/skills/ben-ux-writing'
    ...
    def test_R03_external_link_rejected_by_both_packages(self):
        ...
        (root/'plugin/skills/ben-ux-writing/references/fake.txt').symlink_to(secret)
    ...
    def test_R04_unknown_export_omitted_from_both_packages(self):
        ...
        for archive in (self.base/'out').glob('*.zip'):
            with zipfile.ZipFile(archive) as z:
                ...
                if archive.name.startswith('ben-ux-writing-os-'):
    ...
    def test_R05_score_mutation_stops_build_and_validate(self):
        ...
        before=(root/'plugin/skills/ben-ux-writing/references/07_EVALUATOR.md').read_bytes()
        self.rejected(run(root,'build.py'));self.rejected(self.validate(root))
        self.assertEqual(before,(root/'plugin/skills/ben-ux-writing/references/07_EVALUATOR.md').read_bytes())
    ...
    def test_R12_second_commit_failure_rolls_back_fresh_install(self):
        ...
        def fail_second(src,dst):
            if Path(dst)==self.project/'.claude/skills/ben-ux-writing' and '.chihun-install-staging' in Path(src).parts:
    ...
    def test_R13_second_commit_failure_restores_both_existing_installs(self):
        ...
        for host in ('.agents','.claude'):
            (self.project/host/'skills/ben-ux-writing/USER.txt').write_text(host)
        before={host:install.fingerprint(self.project/host/'skills/ben-ux-writing') for host in ('.agents','.claude')}
        original=os.replace
        def fail_second(src,dst):
            if Path(dst)==self.project/'.claude/skills/ben-ux-writing' and '.chihun-install-staging' in Path(src).parts:
                raise OSError('INJECTED SECOND COMMIT FAILURE')
        ...
        for host,digests in before.items():
            self.assertEqual(install.fingerprint(self.project/host/'skills/ben-ux-writing'),digests)
        self.assertFalse((self.project/'.chihun-install.lock').exists())

    def test_R14_source_root_link_rejected(self):
        root=self.fork();skill=root/'plugin/skills/ben-ux-writing';saved=root/'real-skill'
    ...
    def test_R24_zip_extract_rebuild_and_validate(self):
        ...
        archive=self.base/'out'/('ben-ux-writing-os-v'+common.version(root)+'.zip')
    ...
    def test_R27_missing_reference_blocks_install(self):
        root=self.fork();(root/'plugin/skills/ben-ux-writing/references/07_EVALUATOR.md').unlink()
        self.rejected(run(root,'install.py','--target','both','--project',self.project,'--apply'))
        self.assertEqual(list(self.project.iterdir()),[])

    def test_R28_changed_runtime_blocks_install(self):
        root=self.fork();p=root/'plugin/skills/ben-ux-writing/SKILL.md';p.write_text(p.read_text()+'\nFAKE_UNBUILT_EDIT\n')
        self.rejected(run(root,'install.py','--target','both','--project',self.project,'--apply'))
        self.assertEqual(list(self.project.iterdir()),[])
    ...
    def test_R29_build_failed_rollback_retains_recovery_files(self):
        ...
        old_skill=(root/'plugin/skills/ben-ux-writing/SKILL.md').read_bytes()
        ...
        backups=list(root.glob('.chihun-build-*/old/plugin/skills/ben-ux-writing/SKILL.md'))
    ...
    def test_R30_installer_failed_rollback_retains_backup(self):
        install.install_skill(self.source,self.project,['codex','claude'])
        dest=self.project/'.agents/skills/ben-ux-writing'
