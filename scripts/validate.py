...
def main():
 ...
 skill=ROOT/'plugin/skills/ben.lee-ux-writing';skill_text=(skill/'SKILL.md').read_text(encoding='utf-8')
 fm=re.match(r'^---\n(.*?)\n---\n',skill_text,re.S)
 check('skill_metadata',bool(fm and re.search(r'^name: ben.lee-ux-writing$',fm.group(1),re.M) and re.search(r'^description: .+',fm.group(1),re.M)))
 ...
 check('plugin_minimal_documented_fields',plugin.get('name')=='ben.lee-ux-writing' and plugin.get('version')==version and isinstance(plugin.get('description'),str) and plugin.get('$schema')=='https://agent-plugins.org/schemas/1.0.0/plugin.schema.json','Local required-field check, not remote schema certification')
 ...
 check('version_alignment',build['version']==version==load('plugin/skills/ben.lee-ux-writing/references/RULE_REGISTRY.json')['version'] and (skill/'references/VERSION').read_text().strip()==version)
